...
char ROMlib_rcsid_aspi[] =
		"$Id: oldaspi.c 63 2004-12-24 18:19:43Z ctm $";
#endif
...
PUBLIC BOOLEAN get_aspi_info( aspi_iterator_t *aip, aspi_info_t *aspi_info_p )
{
  ...
  if (has_aspi) {
	...
	if (memory_template.srb.status == NO_ERROR &&
	    (memory_template.u.gdt.device_type == DIRECT_ACCESS_DEVICE ||
	     memory_template.u.gdt.device_type == READ_ONLY_DIRECT_ACCESS_DEVICE)) {
	  ...
	  if (memory_template.srb.status != NO_ERROR ||
	      (memory_template.u.gddi.drive_flags & INT13_MASK)
...
/**/return -1;
  ...
  while (num_bytes != 0) {
    ...
    memory_template.srb.flags   = ((op == READ)
				   ? DIR_TARGET_TO_HOST : DIR_HOST_TO_TARGET);
    memory_template.srb.reserved = 0;
