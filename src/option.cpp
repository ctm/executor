...
char ROMlib_rcsid_option[] = "$Id: option.c 63 2004-12-24 18:19:43Z ctm $";
#endif
...
void Executor::opt_init(void)
{
}

void Executor::opt_shutdown(void)
{
    if(wrap_buf)
    {
        free(wrap_buf);
...
void _safe_strncat(char *dst, const char *src, int len)
{
    char *t;
    int dst_len;
...
void send_to_help_buf(string text, int append_newline_p)
{
    send_to_help_buf(text.c_str(), text.length(), append_newline_p);
}

void _generate_help_message(void)
{
    int i;
    char *buf;
...
void Executor::opt_put_val(opt_database_t &db, const string &opt, string val,
...
void Executor::opt_put_int_val(opt_database_t &db, const string &opt, int valint,
                               priority_t pri, int temp_val_p)
{
    char *val, buf[256];
