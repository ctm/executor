...
static pixel_image_desc_t image_desc;

static int orig_style_p;
static int parsing_p;
...
{
    fprintf(o_fp, "unsigned char %s_%d_bits[] =\n", map_name, depth);
    fprintf(o_fp, "{\n");
}
...
read_string()
{
    ...
    {
        if(i == size)
            string_base = (char *)xrealloc(string_base, size *= 2);
...
read_idn()
{
    ...
    size = 64;
    idn_base = (char *)xmalloc(size);

    i = 0;
    do
    {
        if(i == size)
            idn_base = (char *)xrealloc(idn_base, size *= 2);
...
read_num(int *err)
{
    ...
    size = 64;
    num_base = (char *)xmalloc(size);

    i = 0;
    do
    {
        if(i == size)
            num_base = (char *)xrealloc(num_base, size *= 2);
