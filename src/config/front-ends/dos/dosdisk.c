...
char ROMlib_rcsid_dosdisk[] =
	    "$Id: dosdisk.c 63 2004-12-24 18:19:43Z ctm $";
#endif
...
/*-->*/	    return -1;
	  *flagsp |= DRIVE_FLAGS_FLOPPY;
      ...
      warning_fs_log
	("error = %d, track_sec = %d, max_head = %d, num_drives = %d",
...
enum { CD_DRIVE_NOT_READY = 21 };

#define CD_READ_SECTOR 0x1508

enum { NO_MEDIA_IN_DRIVE = 0x31 };
      ...
      if (dcache_read (disk | DOSFDBIT, buf, d->fpos, block_size, NULL))
	{
	  ...
	  for (error_tries_left = 3; error_tries_left > 0; error_tries_left--)
	      ...
	      if (!changed)
		{
	      ...
	      warning_fs_log
		("sectors_to_read = %d, track = %d, sector = %d, head = %d\n"
...
/*-->*/ return -1;
  ...
  while (num_bytes != 0)
      ...
      for (error_tries_left = 3; error_tries_left > 0; error_tries_left--)
	{
	  ...
	  warning_fs_log
	    ("sectors_to_write = %d, track = %d, sector = %d, head = %d\n"
	     "disk = %d, error = %d, regs.h.al = %d, regs.h.ah = %d, "
	     "regs.x.flags = 0x%x",
