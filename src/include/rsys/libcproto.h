...
typedef struct
{ /* NOTE: we really don't use any fields of this */
} Urusage_t;

typedef struct
{ /* NOTE: the only place we use this is in a .m file */
} Usigcontext_t;
...
typedef struct
{ /* NOTE: we don't use any fields of this puppy */
} * UDBM_t;
...
typedef struct
{ /* NOTE: another one we don't peek inside */
} UDIR_t;
...
extern int (*Usignal(LONGINT sig, LONGINT (*func)(LONGINT signo, LONGINT code,
                                                  Usigcontext_t *scp)))();
