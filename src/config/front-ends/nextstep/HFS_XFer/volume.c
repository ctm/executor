...
#define VOLUMEINFOBACKUP(vcbp) ((vcbp->vcbNmAlBlks * vcbp->vcbAlBlkSiz) + (vcbp->vcbAlBlSt * PHYSBSIZE))
...
PRIVATE HVCB *vcbbyname(StringPtr name)
{
    ...
    for(vcbp = (HVCB *)VCBQHdr.qHead; vcbp && !EqualString(vcbp->vcbVN, name, false, true);
...
PUBLIC HVCB *findvcb(short vrefnum, StringPtr name, LONGINT *diridp)
{
    ...
    vcbp = 0;
    if(namelen && name[1] != ':' && (colonp = indexn((char *)name + 2, ':', namelen - 1)))
    {
...
PUBLIC OSErr myPBSetVInfo(HVolumeParam *pb, BOOLEAN async)
{
    ...
    if(vcbp)
    {
        ...
        else
        {
            ...
            vcbp->vcbAtrb = (vcbp->vcbAtrb & ~ATRBMASK) | (pb->ioVAtrb & ATRBMASK);
