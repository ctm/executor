                  ...
                  LONGINT dstdirid, Str255 s, BOOLEAN doit)
{
    ...
    if(hpb.fileParam.ioFlAttrib & ISDIRMASK)
    {
        ...
        if(doit)
        {
            ...
            if(err == dupFNErr)
            {
                if(!(verify_flags & VERIFY_OVERWRITE_FOLDER) || ask("\poverwrite directory", s) == OK)
                {
                  ...
                  LONGINT dstdirid, Str255 s, BOOLEAN doit)
{
    ...
    else
    {
        ...
        if(pb.volumeParam.ioVAtrb & VOLLOCKEDMASK)
        {
            if(doit)
            {
                ParamText((StringPtr) "\pFiles can not be moved from a locked disk.", 0, 0, 0);
                StopAlert(ONEPARAMALERT, (ProcPtr)0);
            }
            /*-->*/ return false;
        }
