...
char ROMlib_rcsid_vgavdriver[] =
	    "$Id: vgavdriver.c 63 2004-12-24 18:19:43Z ctm $";
#endif
  ...
  if (!old_busy_p && right > left && bottom > top)
    {
       ...
       */
      VGA_IF_SEL (asm ("movw %0,%%es" : : "m" (saved_es)));
      asm ("cld");
...
erase_cursor (void)
{
  ...
  VGA_IF_SEL (asm volatile ("movw %0,%%es" : : "m" (saved_es)));
...
draw_cursor (int x, int y)
{
  ...
  if (draw_cursor_p)
...
#if defined (VGA_SCREEN_NEEDS_FAR_PTR)
      asm ("movw %%fs,%0" : "=m" (saved_fs));
      if (VDRIVER_BYPASS_INTERNAL_FBUF_P ())
	asm volatile ("movw %0,%%fs" : : "g" (vga_screen_selector));
      else
	asm ("pushl %ds ; popl %fs");
      asm ("movw %%fs,%0" : "=g" (cursor_restore_selector));
#endif
      ...
      for (h = height; h > 0; h--)
	{
      ...
      VGA_IF_SEL (asm ("movw %0,%%fs" : : "m" (saved_fs)));
    }
  else
    {
	...
	for (xmod = 0; xmod < 2; xmod++)
	  for (y = 0; y < 16; y++)
	      ...
	      for (x = 0; x < 9; x++)
		{
		  ...
		  m0 = (x > 0) ? m_base[x - 1] : 0;
		  m1 = (x < 8) ? m_base[x    ] : 0;
		  cursor_masks.bpp4[xmod][y][x].and_mask =
		    ~((m0 << (8 - (xmod * 4))) | (m1 >> (xmod * 4)));
		  cursor_masks.bpp4[xmod][y][x].xor_mask =
		    ((d0 << (8 - (xmod * 4))) | (d1 >> (xmod * 4)));
		}
	    }
      }
      break;
...
void
vga_update_cursor (int hot_x, int hot_y)
{
  int x, y;
...
host_flush_shadow_screen (void)
{
  ...
  if (screen_update_func == update_packed_no_paging
      ...
      if (last_refreshed_screen == NULL)
	{
      ...
      else if (find_changed_rect_and_update_shadow
	       ((const uint32 *)vdriver_fbuf, last_refreshed_screen,
...
void
vdriver_set_up_internal_screen (void)
{
