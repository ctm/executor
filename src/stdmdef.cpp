...
void
Executor::cleanup_icon_info (icon_info_t *info)
{
  if (info->icon
      && info->color_icon_p)
    {
...
int
Executor::get_icon_info (mextp item_info, icon_info_t *info, int need_icon_p)
{
  Handle h = NULL;
...
#define MSWTEST
  ...
  th = (tableHandle) NewHandle((Size) sizeof (table) +
								count * sizeof (struct table::tableentry));
  HLock((Handle) th);
  tp = MR(*th);
