...
static char isejectable(const char *dname, long fd)
{
    ...
    if(!retval)
    {
        ...
        sr.sr_ioto = 1;
        if(ioctl(fd, SGIOCREQ, &sr) == 0 && sr.sr_io_status == 0 && inqp->ir_removable)
            retval = true;
    }
#endif
    return retval;
                    ...
                    struct mode_sel_data *msdp)
{
    ...
    ;
    sr.sr_dma_dir = opcode == C6OP_MODESELECT ? SR_DMA_WR : SR_DMA_RD;
...
typedef enum { MODEUNCHANGED = 0,
               MODE800K = 0x22,
               MODE1400K = 0x28 } mode_t;
...
static void trywithmode(char *argv0, char *rawname, mode_t mode)
{
    ...
    openit = false;
    if((fd = open(name, O_RDONLY)) >= 0 && moderead(mode, fd, (char *)buf, sizeof(buf)) == sizeof(buf))
    {
        ...
        if(buf[2][0] == 'B' && buf[2][1] == 'D')
        {
            ...
            alblksiz = (buf[2][20] << 24) | (buf[2][21] << 16) | (buf[2][22] << 8) | buf[2][23];

            if(nmalblks * alblksiz <= 1024L * 1024L * 3)
                newmode = 0666;
        }
        else if((buf[1][0] == PARMAPSIG0 && buf[1][1] == PARMAPSIG1) || (buf[1][0] == OLDMAPSIG0 && buf[1][1] == OLDMAPSIG1))
            openit = true;
