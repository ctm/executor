...
typedef struct
{
    ...
    uint16_t keyflags PACKED; /* Flags for modifier keys (shift, etc. */
...
#define DOSEVQ_QUEUE_TYPE(nelts)                                                  \
    struct                                                                        \
    {                                                                             \
        uint16_t qhead, qtail; /* Head and tail indices.           */               \
        int16_t mouse_dx, mouse_dy; /* Delta mouse position since last query */     \
        uint16_t mouse_interrupt_pending_mask; /* mask for different interrupts. */ \
        uint16_t padding; /* to keep queue long-aligned. */                         \
...
extern uint16_t _get_queue_mem16(unsigned long offset);
extern void _set_queue_mem16(unsigned long offset, uint16_t val);
...
extern uint32_t dosevq_qelt(int which_elt);
...
extern void dosevq_set_qelt(int which_elt, uint32_t e);
