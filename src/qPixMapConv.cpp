...
void
Executor::pixmap_black_white (const PixMap *pixmap,
...
void
Executor::gd_black_white (GDHandle gdh,
		uint32 *black_return, uint32 *white_return)
{
  GDPtr gd;
  PixMapPtr gd_pmap;
...
void
Executor::pixmap_free_copy (PixMap *pm)
{
  DisposPtr (MR (pm->baseAddr));
}
 
void
Executor::pixmap_copy (const PixMap *src_pm, const Rect *src_rect,
...
void
Executor::canonical_from_bogo_color (uint32 index,
			   const rgb_spec_t *rgb_spec,
			   uint32 *pixel_out,
			   RGBColor *rgb_out)
{
...
void
Executor::convert_pixmap (const PixMap *src, PixMap *dst,
		const Rect *rect,
		CTabPtr conv_table)
{
  GDHandle the_gd;
