...
struct Single_descriptor { GUEST_STRUCT;
    GUEST< ULONGINT> id;
    GUEST< ULONGINT> offset;
    GUEST< ULONGINT> length;
};

struct Single_header { GUEST_STRUCT;
    GUEST< LONGINT> magic;
...
struct Single_dates { GUEST_STRUCT;
    GUEST< LONGINT> crdat;
...
struct Single_finfo { GUEST_STRUCT;
    GUEST< FInfo> finfo;
    GUEST< FXInfo> fxinfo;
};
...
typedef struct defaulthead { GUEST_STRUCT;
    GUEST< Single_header> head;
    GUEST< Single_descriptor[10]> desc;    /* we use 4, 6 for spare */
} defaulthead_t;

typedef struct defaultentries { GUEST_STRUCT;
