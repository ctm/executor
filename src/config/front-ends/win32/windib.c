...
{
    NUM_COLORS = 256
}; /* Fix ourselves at an 8-bit palette */
enum
{
    PAL_SIZE = (NUM_COLORS * sizeof(RGBQUAD))
}; /* Size of 8-bit palette */
...
bool vdriver_init(int _max_width, int _max_height, int _max_bpp,
...
{
    /* Verify the video mode parameters */
    if(width && ((width < vdriver_dib_modes.size[0].width) || (width > vdriver_dib_modes.size[1].width)))
        return (false);
    if(height && ((height < vdriver_dib_modes.size[0].height) || (height > vdriver_dib_modes.size[1].height)))
        return (false);
    if(bpp && (bpp != 8))
        return (false);
