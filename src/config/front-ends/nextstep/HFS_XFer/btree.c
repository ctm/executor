...
#define BTENTRY(btp, n) \
    ((anykey *)((char *)(btp) + (((short *)((char *)(btp) + PHYSBSIZE - sizeof(short)))[-(n)])))
...
PRIVATE OSErr maketrailentrybusy(trailentry *tep, INTEGER refnum)
{
    ...
    if(tep->cachep->refnum != refnum || tep->cachep->logblk != tep->logbno || tep->cachep->vptr != vcbp)
...
PRIVATE OSErr deleteroot(cacheentry *oldrootp, cacheentry *block0cachep)
{
    ...
    block0p = (btblock0 *)block0cachep->buf;
    block0p->height--;
    block0p->root = *(ulong *)DATAPFROMKEY(BTENTRY((btnode *)oldrootp->buf, 0));
 ...
 */
            ...
            tep > btpb->trail + 1 && tep->after == 0; --tep)
        {
            ...
            if((*btpb->fp)(&btpb->tofind,
                           BTENTRY((btnode *)tep[-1].cachep->buf, 0))
               == same)
            {
...
PRIVATE OSErr getnewnode(cacheentry **newcachepp, cacheentry *leftp)
{
    ...
    *newcachepp = newcachep;
    ...
    checkbtp(leftbtp);
#endif /* CATFILEDEBUG */
    bcopy(leftbtp, newbtp, sizeof(leftbtp->ndFLink) + sizeof(leftbtp->ndBLink) + sizeof(leftbtp->ndType) + sizeof(leftbtp->ndLevel));
        ...
        else
        {
            ...
            for(done = false; !done;)
            {
                ...
                else if(!done)
                {
                    ...
                    if(frp->cdrType == FILETYPE || (!filesonly && frp->cdrType == DIRTYPE))
                    {
                        if(--count == 0)
                            done = true;
                    }
