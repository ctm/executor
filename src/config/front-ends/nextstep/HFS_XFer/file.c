                         ...
                         BOOLEAN writefcbp)
{
    ...
    nallocneeded = ((LONGINT)neweof - (LONGINT)fcbp->fcbPLen) / vcbp->vcbAlBlkSiz;
    xp = fcbp->fcbExtRec;
    ...
    {
        if((nallocneeded > vcbp->vcbFreeBks) && (alloc == seteof || alloc == alloccontig))
        {
            neweof = fcbp->fcbPLen;
            err = dskFulErr;
            goto done;
                         ...
                         short *refnump)
{
    ...
    for(; fcbp < efcbp; fcbp = (filecontrolblock *)((char *)fcbp + FSFCBLen))
        if(fcbp->fcbVPtr == vcbp && fcbp->fcbFlNum == flnum && ((fcbp->fcbMdRByt & WRITEBIT) || permp == &temp))
            switch(*permp)
            {
                case fsCurPerm:
