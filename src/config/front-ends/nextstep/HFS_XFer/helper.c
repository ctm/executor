...
PUBLIC OSErr updatefloppy(void)
{
    ...
    for(i = 0; i < NBIGBLOCKS; ++i)
    {
        ...
        {
            offset = i * BIGBLOCK;
            if(lseek(floppyfd, offset, L_SET) < 0 || write(floppyfd, buf + offset, BIGBLOCK) != BIGBLOCK)
                err = ioErr;
        }
    }
    return err;
                          ...
                          Ptr bufp, accesstype rw, long *actp)
{
...
#if 1 && !defined(UNIX)
    if(rw != reading && strncmp((char *)vcbp->vcbVN, "\pMyVol", vcbp->vcbVN[0] + 1) != 0)
    {
    ...
    if(firstbigblock >= 0 || lastbigblock < NBIGBLOCKS)
    {
        for(i = firstbigblock; i <= lastbigblock; ++i)
        {
            ...
            {
                offset = i * BIGBLOCK;
                if(lseek(floppyfd, offset, L_SET) < 0 || read(floppyfd, buf + offset, BIGBLOCK) != BIGBLOCK)
                {
                    err = ioErr;
                    break;
                }
