...
char ROMlib_rcsid_qColorutil[] = "$Id: qColorutil.c 63 2004-12-24 18:19:43Z ctm $";
#endif
...
/* ctab with `zero' as the seed; if StdBits encounters a source pixmap
   ...
   performed */
CTabHandle Executor::ROMlib_dont_depthconv_ctab;

/* black and white color table; element zero is always white, elt one
   always black

   this colortable is always valid */
CTabHandle Executor::ROMlib_bw_ctab;

CTabHandle Executor::no_stdbits_color_conversion_color_table;
...
CTabHandle
Executor::validate_fg_bk_ctab(void)
{
    int pixel_size;
...
CTabHandle
Executor::validate_relative_bw_ctab(void)
{
    GDHandle gd;
    PixMapHandle gd_pmap;
...
void Executor::ROMlib_copy_ctab(CTabHandle src, CTabHandle dst)
{
    int ctab_size;
