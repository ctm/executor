...
Call68KProc_from_native(void *addr, ProcInfoType info, va_list ap)
{
    ...
    if(convention == kPascalStackBased || convention == kD0DispatchedPascalStackBased || convention == kD1DispatchedPascalStackBased || convention == kStackDispatchedPascalStackBased)
    {
        argp = params;
        incr = 1;
...
CallUniversalProc_from_native(UniversalProcPtr proc, ProcInfoType info, ...)
{
    ...
    if(proc->goMixedModeTrap != (uint16)CWC(MIXED_MODE_TRAP))
        ...
        retval = CallUniversalProc_from_native_common(ap, args_via_stdarg,
