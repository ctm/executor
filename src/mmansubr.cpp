...
void
Executor::mm_set_block_fields_offset (block_header_t *block,
		     unsigned state, unsigned use,
		     unsigned size_correction,
...
/* Given a block BLOCK big enough to hold a block of size SIZE, split
   ...
   field. */
void
Executor::ROMlib_setupblock (block_header_t *block,
...
void 
Executor::ROMlib_coalesce (block_header_t *block)
{
...
void 
Executor::ROMlib_freeblock (block_header_t *block)
{
  THz current_zone;
...
/* Move the relocatable block at OLDL to the empty block NEWL, growing
   its size to NEWSIZE */
void
Executor::ROMlib_moveblock (block_header_t *oldl, block_header_t *newl,
		  uint32 newsize)
{
  Handle master;
  THz current_zone;
...
/* Find the total amount of free space starting at block.  Compress them
   all. */
int32 
Executor::ROMlib_amtfree (block_header_t *block)
{
  int32 total;
  block_header_t *b;
  THz current_zone;
...
OSErr
Executor::ROMlib_relalloc (Size size, block_header_t ** newblk)
{
  block_header_t *b;
